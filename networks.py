...
file = 'data/adj_allVillageRelationships_HH_vilno_3.csv'
...
class Vill_Net:
    ...
    def plot_graph(self, show = True):
        ...
        color_dict = {1: "green", 0: "blue"}
        visual_style["vertex_color"] = [color_dict[take_up] for take_up in self._g.vs["take_up"]]
        fig, ax = plt.subplots()
        plt.axis('off')
        igraph.plot(self._g, **visual_style, target=ax)
        if show:
            plt.show()